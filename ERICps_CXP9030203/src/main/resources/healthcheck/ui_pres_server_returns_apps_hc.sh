...
for ui_instance in ${LITP_SU_LIST}; do
	LITP_FULL_URL="${LITP_TREE_BASE}/${LITP_COMPONENT_NAME}/${ui_instance}/jee/cfg/ip/"
	...
	CURL_COMMAND="${CURL} -s -H ${CURL_HEADER} http://${UI_INSTANCE_IP_ADDRESS}:8080/rest/apps"
	TOKEN=$( ${CURL_COMMAND} | ${GREP} -o ${CITRIX_APP_PATTERN} )
...
if [ ! -z "${LITP_SU_ERROR_LIST}" ]; then
	graceful_exit 1 "No UI applications list received from these JBoss UI instances on node ${NODE_NAME}: ${LITP_SU_ERROR_LIST}"
else
	graceful_exit 0 "UI applications list:{ ${TOKEN} } received from all UI JBoss instances on node ${NODE_NAME}: ${LITP_SU_SUCCESS_LIST}"
fi
